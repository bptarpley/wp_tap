                    ...
                    fetch(`${this.tap.host}/api/corpus/${this.tap.corpus_id}/Event/?f_artifacts.id=${this.artID}&only=id,label`)
                        ...
                        .then(exhibitInfo => {
                            ...
                            if (exhibitInfo.records) {
                                ...
                                let exhibitLinks = exhibitInfo.records.map(ex => `<a href="/ephemera?facet=exhibits&value=${ex.id}">${ex.label}</a>`)

                                metas.push(`<dt>${exhibitLabel}:</dt><dd>${exhibitLinks.join('<br />')}</dd>`)
                            }

                            if (art.agents.length) {
                                let peopleLinks = art.agents.map(agent => `<a href="/ephemera?facet=agents&value=${agent.id}">${agent.label}</a>`)
                                metas.push(`<dt>People:</dt><dd>${peopleLinks.join('<br />')}</dd>`)
                            }

                            if (art.collection) metas.push(`<dt>Collection:</dt><dd><a href="/ephemera?facet=collections&value=${art.collection.id}">${art.collection.label}</a></dd>`)
                            if (art.media_type) metas.push(`<dt>Media Type:</dt><dd><a href="/ephemera?facet=media&value=${art.media_type.id}">${art.media_type.label}</a></dd>`)

                            if (art.themes.length) {
                                let themeLinks = art.themes.map(theme => `<a href="/ephemera?facet=themes&value=${theme.id}">${theme.label}</a>`)
                                metas.push(`<dt>Themes:</dt><dd>${themeLinks.join('<br />')}</dd>`)
                            }

                            if (art.year) metas.push(`<dt>Year:</dt><dd><a href="/ephemera?facet=years&value=${art.year}">${art.year}</a></dd>`)
